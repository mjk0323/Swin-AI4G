...
WEAPON_MODES = {
	pyglet.window.key._1: 'rifle',
	pyglet.window.key._2: 'rocket',
	pyglet.window.key._3: 'hand_gun',
	pyglet.window.key._4: 'hand_grenade',
}
...
class Projectile(object):
	...
	def __init__(self, world=None, scale=30.0, mass=1.0, mode='rifle'):
		...
		self.initial_target_pos = Vector2D(world.target.pos.x, world.target.pos.y)
		self.initial_target_vel = Vector2D(world.target.vel.x, world.target.vel.y)
		...
		self.target_pos = Vector2D(world.target.pos.x, world.target.pos.y)

		direction = (self.target_pos - self.pos).get_normalised()
	...
	def rifle(self, speed):
		...
		target_pos = self.initial_target_pos
		target_vel = self.initial_target_vel
	...
	def hand_gun(self, speed):
		...
		target_vel = self.world.target.vel
