...
def SearchWALKER(graph, source_idx, target_idx, limit=0):
...
def SearchSWIMMER(graph, source_idx, target_idx, limit=0):
	''' A* Search. Expand the minimum path cost-so-far + lowest heuristic cost. '''
	closed = set() # set - of visited nodes
	route = {} # dict of {to:from} items to find our way home
	open = PriorityQueue() # priority queue of the current leaf edges
...
def SearchHIKER(graph, source_idx, target_idx, limit=0):
	''' A* Search. Expand the minimum path cost-so-far + lowest heuristic cost. '''
	closed = set() # set - of visited nodes
	route = {} # dict of {to:from} items to find our way home
	open = PriorityQueue() # priority queue of the current leaf edges
...
def SearchCLIMBER(graph, source_idx, target_idx, limit=0):
	''' A* Search. Expand the minimum path cost-so-far + lowest heuristic cost. '''
	closed = set() # set - of visited nodes
	route = {} # dict of {to:from} items to find our way home
	open = PriorityQueue() # priority queue of the current leaf edges
...
if __name__ == '__main__':
	...
	# try WALKER Search
	print('from 5 to 3 WALKER:')
	
	print(SearchWALKER(g,5,3))
	# try SWIMMER Search
	print('from 5 to 3 SWIMMER:')
	print(SearchSWIMMER(g,5,3))
	...
	print(g.get_adj_list_str())
	# HIKER search
	print('from 5 to 3 HIKER:')
	print(SearchHIKER(g,5,3))
	# CLIMBER Search
	if False:
		g.cost_h = SimpleTestHeuristic
		print('from 1 to 3 CLIMBER:')
		print(SearchCLIMBER(g,1,3))
